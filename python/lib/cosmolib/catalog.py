...
class DataCatalog(object):
    ...
    def to_distr(self, limit, nbins):
        ...
        + bins_z: int, list tuple, ndarray
            Binning scheme for redshift distribution
        Outputs:
    ...
    def redshift_distr(self, limit, nbins, weighted=False, normed=False, cosmo=None):
        """ Calculate redshift distribution. If cosmology is given, convert
        into comoving distribution
        Inputs:
        + limit: list, tuple, ndarray, dict
            Binning range for redshift distribution. If dict, use value of key 'z'.
        + nbins: int
        ...
        + cosmo: cosmology.Cosmology (default=None)
            Cosmology object to convert redshift to comoving distance. If not
            None, return redshift distribution to comoving distance distribution
        Outputs:
        + z_distr: ndarray
            Redshift (comoving) distribution
        + bins_z: ndarray
            Redshift (comoving) distribution bins """
        bins_range = limit['z'] if isinstance(limit, dict) else limit

        weights = self.catalog[:, 3] if weighted else None
        z_distr, bins_z = numpy.histogram(
            self.catalog[:, 2], bins=nbins, range=bins_range, weights=weights)

        # Normalized by number of galaxies
        if normed:
            z_distr = 1.*z_distr/self.ntotal

        # Convert to redshift distribution
        if cosmo is not None:
            bins_z = cosmo.z2r(bins_z)
    ...
    def s_distr(self, s_max, nbins_s, cosmo, job_helper=None, leaf=40):
...
class DistrCatalog(object):
    ...
    def __init__(self, reader=None):
        """ Initialize angular and redshift distribution
        Inputs:
        ...
        if reader is not None:
            ...
            with numpy.load(reader['path']) as f:
                self.z_distr = f[reader['z_distr']]
                self.angular_distr = f[reader['angular_distr']]
                self.bins_z = f[reader['bins_z']]
    ...
    def _z_theta_distr_thread(self, theta_max, nbins_theta, data_catalog, balltree,
                              job_helper, mode):
        """ Thread function of self.z_theta_distr
        Outputs:
        + r_theta_distr: ndarray """

        # Initialize comoving-angular distribution
        nbins_z = self.bins_z.size-1
        limit = ((0., theta_max), (self.bins_z.min(), self.bins_z.max()))
        z_theta_distr = numpy.zeros((2, nbins_theta, nbins_z))
        ...
        return z_theta_distr

    def z_theta_distr(self, data, theta_max, nbins_theta, job_helper=None, mode=None, leaf=40):
        """ Calculate comoving-angular distribution.
        Use a modified nearest-neighbors BallTree algorithm to calculate
        angular distance up to a given angle. Metric = 'haversine'.
        NOTE: assume self.bins_z is uniformed
        Inputs:
        + data: DataCatalog
